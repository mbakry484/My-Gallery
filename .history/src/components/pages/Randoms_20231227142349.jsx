...
const Randoms = () => {
  ...
  {data.imgSrc && (
    ...
    >
      <button style={{ position: 'absolute', top: '10px', right: '10px', backgroundColor: 'red' }} onClick={() => imgAction('close')}>
        X{' '}
      </button>

      <button style={{ position: 'absolute', top: '48%', left: '10px', backgroundColor: 'green' }} onClick={() => imgAction('prev-img')}>
        Prev{' '}
      </button>

      <img src={data.imgSrc} style={{ width: 'auto', maxWidth: '100%', maxHeight: '100%', borderRadius: '0.5' }} />

      <button style={{ position: 'absolute', top: '48%', right: '10px', backgroundColor: 'green' }} onClick={() => imgAction('next-img')}>
        Next{' '}
      </button>
    </div>
  )}
...
return(
  ...
  <div className='randImgContainer'>
    <div className='randImg'>
      {
      imagesArray.map((imgSrc,index)=>(<img key={index} src={imgSrc} onClick={() => viewImage(imgSrc, index)} />))
      }    
      </div>
  </div>
  </>
