...
const Randoms = () => {
  ...
  const [imagesArray,setImagesArray] = use[
    img3, img4, img5,
  ...
  const deleteImg = (imgSrc) => {
    if (data.imgSrc && deleteMode) {
      const updatedArray = imagesArray.filter((img) => img !== imgSrc);
      setData({ imgSrc: '', index: 0 });
      setImagesArray(updatedArray); // You need to have a state for imagesArray in Randoms
    } else {
      // Open the image or perform other actions
      setData({ imgSrc, index: imagesArray.indexOf(imgSrc) });
    }
  };
  ...
  return (
    <>
      ...
      <div className="sunImgsContainer">
        ...
        <PagesHeader onDelete={() => deleteImg(data.imgSrc)} />
          ...
          <img src={imgSrc} id="item" alt="" onClick={() => deleteImg(imgSrc)} />
        </div>
